...
def custom_dilation(img, kernel_size):
    """
    Perform morphological dilation on a grayscale image.
    ...
    for y in range(h):
        for x in range(w):
            # Define the neighborhood bounds (handling borders).
            y_start = max(0, y - pad)
            y_end = min(h, y + pad + 1)
            x_start = max(0, x - pad)
            x_end = min(w, x + pad + 1)
            ...
            # Assign the maximum value found in the region.
            out[y, x] = np.max(roi)
...
def run():
    """
    Main function for Exercise 03b (Dilation).

    Usage:
      python exercise_03b.py <kernel_parameter> <input_image> <output_image>
    where:
    ...
    If no arguments are provided, default values are used:
      ...
      output_image:     ./output/immed_gray_inv_dil.pgm
    """
    ...
    default_output = os.path.join(script_dir, "output", "immed_gray_inv_dil.pgm")
    ...
    if len(sys.argv) == 1:
        ...
        print("Usage: python exercise_03b.py <kernel_parameter> <input_image> <output_image>")
        sys.exit(1)
    ...
    # Perform dilation.
    dilated_img = custom_dilation(img, kernel_size)
    print(f"Dilation applied with a {kernel_size}x{kernel_size} kernel.")

    os.makedirs(os.path.dirname(output_image_path), exist_ok=True)

    cv2.imwrite(output_image_path, dilated_img)
