...
def main():
    """
    Checks the idempotence of the "closing–opening" alternated filters.
    
    Usage:
      python exercise_07a.py <i> <input_image> <output_text_file>
    ...
    If no arguments are provided, default values are used:
      ...
      - output_text_file: ./output/exercise_07a_result.txt
    """
    ...
    if len(sys.argv) == 1:
        ...
        output_text_path = os.path.join(script_dir, "output", "exercise_07a_result.txt")
        print("No arguments provided. Using default values:")
        print("  i:", i, "-> kernel size =", 2*i+1)
        print("  Input image:", input_image_path)
        print("  Output text file:", output_text_path)
    elif len(sys.argv) < 4:
        print("Usage: python exercise_07a.py <i> <input_image> <output_text_file>")
        sys.exit(1)
    else:
        i = int(sys.argv[1])
    ...
    output_img1 = os.path.join(script_dir, "output", "exercise_07a_output_01.pgm")
    output_img2 = os.path.join(script_dir, "output", "exercise_07a_output_02.pgm")
    ...
    with open(output_text_path, "w") as f:
        f.write(f"Closing–Opening idempotence: {identical}\n")

    print(f"Comparison result saved in {output_text_path}")
    print("Closing–Opening idempotence:", identical)
    return identical
